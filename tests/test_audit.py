...
def test_slice_discovery_is_reproducible() -> None:
    ...
    summary_one = SliceDiscoverer(random_state=70, min_samples_leaf=20).fit(
        data.X_ref,
        data.y_ref,
    ...
    summary_two = SliceDiscoverer(random_state=70, min_samples_leaf=20).fit(
        data.X_ref,
        data.y_ref,
        data.reference_predictions,
        data.X_target,
    ...
    assert summary_one["target_failure_share"].tolist() == summary_two["target_failure_share"].tolist()
...
def test_reliability_auditor_report_structure() -> None:
    ...
    assert {"metric", "worst_group", "absolute_gap"}.issubset(
        auditor.disparity_summary().columns
    )
